...
const Princ = () => {
    ...
    useEffect(()=>{
        setpermit(JSON.parse(localStorage.getItem('loggedinuser')).isloggedin && JSON.parse(localStorage.getItem('loggedinuser')).pos==='PRI');
    ...
    const handle = async(e) =>{
        e.preventDefault();
        if(JSON.parse(localStorage.getItem('loggedinuser')).isloggedin && JSON.parse(localStorage.getItem('loggedinuser')).pos!=='PRI') alert('Please logout other user');
        else{
            await axios.get('http://localhost:5000/princelogdetails/'+usr.uname).then((res)=>{
